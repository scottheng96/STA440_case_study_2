...
for (i in 1:15) {
  ...
  EDA_chest_subject_df = data.frame()
  EDA_wrist_subject_df = data.frame()
  EMG_subject_df = data.frame()
  Temp_chest_subject_df = data.frame()
  Temp_wrist_subject_df = data.frame()
  inhale_subject_df = data.frame()
  exhale_subject_df = data.frame()
  resp_subject_df = data.frame()
  breath_subject_df = data.frame()
  BVP_wrist_subject_df = data.frame()
  ECG_chest_subject_df = data.frame()
  
  for (j in 1:3) {
    l <- chest_wrist$Label %>% unique() %>% .[j]
    
    subject_p <- chest_wrist %>% filter(Participant == p & Label == l)
    ...
    # chest EDA
    mean_EDA <- rollapply(subject_p$EDA, 20, mean)
    sd_EDA <- rollapply(subject_p$EDA, 20, sd)
    max_EDA <- rollapply(subject_p$EDA, 20, max)
    min_EDA <- rollapply(subject_p$EDA, 20, min)
    ...
    # wrist EDA
    mean_EDA_wrist <- rollapply(subject_p$EDA_wrist, 20, mean)
    sd_EDA_wrist <- rollapply(subject_p$EDA_wrist, 20, sd)
    max_EDA_wrist <- rollapply(subject_p$EDA_wrist, 20, max)
    min_EDA_wrist <- rollapply(subject_p$EDA_wrist, 20, min)
    ...
    # chest EMG
    mean_EMG <- rollapply(subject_p$EMG, 20, mean)
    sd_EMG <- rollapply(subject_p$EMG, 20, sd)
    max_EMG <- rollapply(subject_p$EMG, 20, max)
    min_EMG <- rollapply(subject_p$EMG, 20, min)
    ...
    # wrist Temp
    mean_Temp_wrist <- rollapply(subject_p$TEMP_wrist, 20, mean)
    sd_Temp_wrist <- rollapply(subject_p$TEMP_wrist, 20, sd)
    max_Temp_wrist <- rollapply(subject_p$TEMP_wrist, 20, max)
    min_Temp_wrist <- rollapply(subject_p$TEMP_wrist, 20, min)
    ...
    # chest Temp
    mean_Temp_chest <- rollapply(subject_p$Temp, 20, mean)
    sd_Temp_chest <- rollapply(subject_p$Temp, 20, sd)
    max_Temp_chest <- rollapply(subject_p$Temp, 20, max)
    min_Temp_chest <- rollapply(subject_p$Temp, 20, min)
    ...
    # inhale
    mean_inhale <- rollapply(subject_p$inhale, 20, mean, na.rm = TRUE)
    sd_inhale <- rollapply(subject_p$inhale, 20, sd, na.rm = TRUE)
    ...
    # exhale
    mean_exhale <- rollapply(subject_p$exhale, 20, mean, na.rm = TRUE)
    sd_exhale <- rollapply(subject_p$exhale, 20, sd, na.rm = TRUE)
    ...
    # breath rate
    mean_breath <- rollapply(subject_p$breath_rate, 20, mean, na.rm = TRUE)
    sd_breath <- rollapply(subject_p$breath_rate, 20, sd, na.rm = TRUE)
    max_breath <- rollapply(subject_p$breath_rate, 20, max, na.rm = TRUE)
    min_breath <- rollapply(subject_p$breath_rate, 20, min, na.rm = TRUE)
    ...
    EDA_chest_subject_df <- bind_rows(EDA_chest_subject_df, EDA_chest_df)
    EDA_wrist_subject_df <- bind_rows(EDA_wrist_subject_df, EDA_wrist_df)
    EMG_subject_df <- bind_rows(EMG_subject_df, EMG_df)
    Temp_chest_subject_df <- bind_rows(Temp_chest_subject_df, Temp_chest_df)
    Temp_wrist_subject_df <- bind_rows(Temp_wrist_subject_df, Temp_wrist_df)
    inhale_subject_df <- bind_rows(inhale_subject_df, inhale_df)
    exhale_subject_df <- bind_rows(exhale_subject_df, exhale_df)
    resp_subject_df <- bind_rows(resp_subject_df, resp_df)
    breath_subject_df <- bind_rows(breath_subject_df, breath_df)
    BVP_wrist_subject_df <- bind_rows(BVP_wrist_subject_df, BVP_wrist_df)
    ECG_chest_subject_df <- bind_rows(ECG_chest_subject_df, ECG_chest_df)
  }
  
  # adding dataframes to list
  EDA_chest_list[[i]] <- EDA_chest_subject_df
  EDA_wrist_list[[i]] <- EDA_wrist_subject_df
  EMG_list[[i]] <- EMG_subject_df
  Temp_chest_list[[i]] <- Temp_chest_subject_df
  Temp_wrist_list[[i]] <- Temp_wrist_subject_df
  inhale_list[[i]] <- inhale_subject_df
  exhale_list[[i]] <- exhale_subject_df
  resp_list[[i]] <- resp_subject_df
  breath_list[[i]] <- breath_subject_df
  BVP_wrist_list[[i]] <- BVP_wrist_subject_df
  ECG_chest_list[[i]] <- ECG_chest_subject_df
}
...
for (i in 1:15) {
  ...
  ACC_chest_subject_df = data.frame()
  ACC_wrist_subject_df = data.frame()
  for (j in 1:3) {
    l <- chest_wrist$Label %>% unique() %>% .[j]
    ...
    ACC_chest_subject_df <- bind_rows(ACC_chest_subject_df, ACC_chest_df)
    ...
    ACC_wrist_subject_df <- bind_rows(ACC_wrist_subject_df, ACC_wrist_df)
    
  }
  
  ACC_chest_list[[i]] <- ACC_chest_subject_df
  ACC_wrist_list[[i]] <- ACC_wrist_subject_df
}
