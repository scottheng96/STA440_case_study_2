...
Section 2 focuses on providing descriptions of the WESAD data, how the raw data was processed and the engineering of various features. Exploratory data analysis plots are included to provide initial insights to our study based on the data. In section 3, we provide an extensive literature review on previous research and discuss how this paper builds on the work from those studies. Section 4 describes our data and domain-driven approaches to feature engineering, model selection and type of evaluation metrics based on the nature of our study and the dataset. Section 5 discusses the results of our models and compares the predictive accuracy of wrist only data models and wrist and chest data models, along with multiple diagnostic techniques to address model assumptions and check model fit. Section 6 consists of interpretations from the study’s models and addressing issues on heterogeneity, collinearity and the predictive significance of various features.
...
\beta_5 \ \sigma^W_{i EDA} * &\sigma^W_{i ACC} + \beta_6 \ \sigma^W_{i HRV} * \sigma^W_{i ACC} \ + \\ 
\sum_{j=2}^{15}\beta_{7j} \ I(subject_i & = j) \ + \sum_{j=2}^{15} \beta_{8j} \  \sigma^W_{i ACC} * I(subject_i = j) , &(1)
\end{aligned}
$$
...
Since there is no standard sampling frequency in the literature, we first examine the sensitivity of our sampling rate of 4 Hz. Table 3 displays the results of the wrist data and all data models (Models 3 and 4), with the same predictors as Models 1 and 2, when using data that was sampled at 1 Hz. In general, Models 1 and 2 perform better on the training data, while Models 3 and 4 perform better on the testing data. There is also higher variability in the metrics of the models using data sampled at 1 Hz - this is because we are using a much smaller sample size, which leads to higher sampling variability. We then compare the estimates of Models 3 and 4 to those of Models 1 and 2, respectively; the estimates for Models 3 and 4 are located in Table XX and XX in the Appendix. Both the wrist-only and combined data models are reasonably robust, since we report similar estimates for the predictors; this, along with the minimal difference in metrics, suggests that our data is not highly sensitive to different sampling frequencies.  
...
```{r}
bind_rows(calc_metrics(lr_model_final_wrist_60window, test_60window, 5) %>% 
            mutate(model = "Wrist Only (Model 5)"), 
          calc_metrics(lr_model_final_wrist_chest_60window, test_60window, 5) %>% 
            mutate(model = "Chest & Wrist (Model 6)")) %>% 
  dplyr::select(model, everything()) %>% 
  kable( booktabs = T,  align = "c",
...
Additionally, we fit wrist-only and combined models (5 and 6) using data that was sampled using a 60 second window size and a 0.25 second window shift (as compared to a 5 second window size and 0.25 second window shift). We trained this set of models using the same predictors that we used in the final models (Model 1 and Model 2). In general, this set of models performed better in terms of evaluation metrics. We then compare the estimates from this set of models to the estimates from our original models; the estimates are displayed in Tables XX and XX in the Appendix. Our wrist-only model appears to be reasonably robust, as most estimates using data sampled using a 60 second window size and a 0.25 second window shift were similar to the estimates from Model 1. However, the combined data model was less robust, as there are estimates, such as the one for the standard deviations of chest EDA that were quite different (1.12 vs. 2.36 for Model 2 vs. Model 6). This disparity could be due to the loss of data when sampling using a 60 second window, or due to the choice to initially downsample our data to 4Hz. 
...
When standard deviation of wrist ACC increases by 1, we expect the odds of being stressed versus amused to multiply by a factor of `r round(exp(0.445), 3)` (with 95% CI `r round(exp(2.76076), 3)`, `r round(exp(4.59924), 3)`), holding all else constant. 
When standard deviation of wrist HRV increases by 1, we expect the odds of being stressed versus amused to multiply by a factor of `r round(exp(4.027), 3)` (with 95% CI `r round(exp(0.3568), 3)`, `r round(exp(0.5332), 3)`), holding all else constant. 


Holding standard deviation of wrist HRV constant, when standard deviation of wrist EDA and wrist ACC both increase by 1, we expect the odds of being stressed versus amused to multiply by an additional factor of `r round(exp(-1.129), 3)` (with 95% CI `r round(exp(-1.18976), 3)`,  `r round(exp(-1.06824), 3)`.
Holding standard deviation of wrist ACC constant, when standard deviation of wrist EDA and wrist HRV both increase by 1, we expect the odds of being stressed versus amused to multiply by an additional factor of `r round(exp(0.5), 3)` (with 95% CI `r round(exp(0.33732), 3)`,  `r round(exp(0.66268), 3)`.
Holding standard deviation of wrist EDA constant, when standard deviation of wrist HRV and wrist ACC both increase by 1, we expect the odds of being stressed versus amused to multiply by an additional factor of `r round(exp(0.253), 3)` (with 95% CI `r round(exp(0.11188), 3)`,  `r round(exp(0.39412), 3)`.


### Chest and Wrist Model

When standard deviation of chest EDA increases by 1, we expect the odds of being stressed versus amused to multiply by a factor of `r round(exp(1.198), 3)` (with 95% CI `r round(exp(1.08432), 3)`, `r round(exp(1.31168), 3)`), holding all else constant. 
When standard deviation of wrist EDA increases by 1, we expect the odds of being stressed versus amused to multiply by a factor of `r round(exp(8.779), 3)` (with 95% CI `r round(exp(8.48892), 3)`, `r round(exp(9.06908), 3)`), holding all else constant. 
When standard deviation of chest temperature increases by 1, we expect the odds of being stressed versus amused to multiply by a factor of `r round(exp(0.171), 3)` (with 95% CI `r round(exp(0.13376), 3)`, `r round(exp(0.20824), 3)`), holding all else constant. 
When minimum wrist temperature increases by 1, we expect the odds of being stressed versus amused to multiply by a factor of `r round(exp(-1.801), 3)` (with 95% CI `r round(exp(-1.899), 3)`, `r round(exp(-1.703), 3)`), holding all else constant. 
When minimum chest EMG increases by 1, we expect the odds of being stressed versus amused to multiply by a factor of `r round(exp(-0.143), 3)` (with 95% CI `r round(exp(-0.18416), 3)`, `r round(exp(-0.10184), 3)`), holding all else constant.
When standard deviation of inhale increases by 1, we expect the odds of being stressed versus amused to multiply by a factor of `r round(exp(0.573), 3)` (with 95% CI `r round(exp(0.52204), 3)`, `r round(exp(0.62396), 3)`), holding all else constant.
When standard deviation of exhale increases by 1, we expect the odds of being stressed versus amused to multiply by a factor of `r round(exp(-0.266), 3)` (with 95% CI `r round(exp(-0.30716), 3)`, `r round(exp(-0.22484), 3)`), holding all else constant.
When maximum breath cycle time increases by 1, we expect the odds of being stressed versus amused to multiply by a factor of `r round(exp(0.66932), 3)` (with 95% CI `r round(exp(0.636), 3)`, `r round(exp(0.60268), 3)`), holding all else constant.
When inhale/exhale ratio increases by 1, we expect the odds of being stressed versus amused to multiply by a factor of `r round(exp(-0.289), 3)` (with 95% CI `r round(exp(-0.32624), 3)`, `r round(exp(-0.25176), 3)`), holding all else constant.
When mean chest ACC increases by 1, we expect the odds of being stressed versus amused to multiply by a factor of `r round(exp(0.039), 3)` (with 95% CI `r round(exp(-0.01), 3)`, `r round(exp(0.088), 3)`), holding all else constant.
When standard deviation of chest ACC increases by 1, we expect the odds of being stressed versus amused to multiply by a factor of `r round(exp(0.802), 3)` (with 95% CI `r round(exp(0.74712), 3)`, `r round(exp(0.85688), 3)`), holding all else constant.
When standard deviation of wrist ACC increases by 1, we expect the odds of being stressed versus amused to multiply by a factor of `r round(exp(0.388), 3)` (with 95% CI `r round(exp(0.31744), 3)`, `r round(exp(0.45856), 3)`), holding all else constant.
When standard deviation of wrist HR increases by 1, we expect the odds of being stressed versus amused to multiply by a factor of `r round(exp(0.209), 3)` (with 95% CI `r round(exp(0.16196), 3)`, `r round(exp(0.25604), 3)`), holding all else constant.
When standard deviation of wrist HRV increases by 1, we expect the odds of being stressed versus amused to multiply by a factor of `r round(exp(0.173), 3)` (with 95% CI `r round(exp(0.1338), 3)`, `r round(exp(0.2122), 3)`), holding all else constant.
When rms of wrist HRV increases by 1, we expect the odds of being stressed versus amused to multiply by a factor of `r round(exp(0.282), 3)` (with 95% CI `r round(exp(0.17028), 3)`, `r round(exp(0.39371), 3)`), holding all else constant.
When standard deviation of chest HR increases by 1, we expect the odds of being stressed versus amused to multiply by a factor of `r round(exp(-0.74), 3)` (with 95% CI `r round(exp(-0.9262), 3)`, `r round(exp(-0.5538), 3)`), holding all else constant.
When standard deviation of chest HR increases by 1, we expect the odds of being stressed versus amused to multiply by a factor of `r round(exp(0.061), 3)` (with 95% CI `r round(exp(0.0218), 3)`, `r round(exp(0.1002), 3)`), holding all else constant.


Additionally, holding all else constant, when standard deviation of wrist HRV and minimum wrist temperature both increase by 1, we expect the odds of being stressed versus amused multiply by an additional factor of `r round(exp(-4.665), 3)` (with 95% CI `r round(exp(-4.8218), 3)`, `r round(exp(-4.5082), 3)`).
...
For the interaction effect between participant and rms wrist HRV, as rms HRV increases, Subject 11's log-odds of being stressed increase more compared to the baseline Subject 10, and Subject 3's log-odds also significantly increase more, although not by as much as Subject 11. Subjects 4 and 16 do not appear to have a significant difference in the impact on stress as rms HRV increases compared to Subject 10. We expect Subjects 2, 5, 6, 8, 9, and 14 all to have a decrease in log-odds of being stressed as rms HRV increases compared to Subject 10, however these subjects do not seem to vary significantly between each other. Lastly, Subjects 7, 13, 15, and 17 all see an even greater decrease in log-odds of being stressed as rms HRV increases compared 2, 5, 6, 8, 9, and 14, but again, and Subject 7 varies significantly from Subjects 15 and 17, but not 13.
...
lr_model_final_wrist_chest_coef_tidy_df %>%
  ...
  mutate(term = chest_wrist_rows,
         ...
         `lower.ci` = ifelse(term == "S10" | term == "$rms^W_{HRV}$ * S10", "", as.numeric(estimate) - 1.96*as.numeric(std.error)),
         `upper.ci` = ifelse(term == "S10" | term == "$rms^W_{HRV}$ * S10", "", as.numeric(estimate) + 1.96*as.numeric(std.error))
  )%>%
...
lr_model_final_wrist_60window$finalModel %>% 
  ...
  kableExtra::kable( booktabs = T,  escape = F, 
                     caption = "Coefficients obtained with Model 5 (Wrist Model, Data Sampled using 60s Window Shift)",
                     col.names = c("Coefficient", "Estimate", "Std. Error", "p-value", "Lower CI", "Upper CI")) %>% 
  kable_styling(latex_options = c("hold_position")) %>%
  kableExtra::pack_rows("Participant", 8,22, escape = F) %>% 
  kableExtra::pack_rows("$\\sigma^W_{ACC}$ * Participant", 23,37, escape = F) %>% 
...
lr_model_final_wrist_chest_60window_coef_tidy_df %>%
  ...
  kableExtra::kable( booktabs = T, escape = F,
                     caption = "Coefficients obtained with Model 6 (Chest and Wrist Model, Data Sampled using 60s Window Shift)",
                     col.names = c("Coefficient", "Estimate", "Std. Error", "p-value", "Lower CI", "Upper CI")) %>%
  kable_styling(latex_options = c("hold_position")) %>%
  kableExtra::pack_rows("Participant", 20,34, escape = F) %>%
  column_spec(1, width = "4cm") %>%
