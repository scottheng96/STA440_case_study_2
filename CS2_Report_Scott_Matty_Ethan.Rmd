...
Section 2 focuses on providing descriptions of the WESAD data, how the raw data was processed and the engineering of various features. Exploratory data analysis plots are included to provide initial insights to our study based on the data. In section 3, we provide an extensive literature review on previous research, put our research in context of them. Section 4 describes our data and domain-driven approaches to feature engineering, model selection and type of evaluation metrics based on the nature of our study and the dataset. Section 5 discusses the results of our models, predictive accuracies of models being trained only on wrist data and on wrist and chest data combined, along with multiple diagnostic techniques to address model assumptions and check model fit. Section 6 consists of interpretations from the study’s models and addressing issues on heterogeneity, collinearity and the predictive significance of various features.
...
```{r}
final_data_60_window <- readRDS("final_data_240s_window.Rds") %>% 
  filter(label != 1) %>% 
  mutate(label = case_when(
    label ==  2 ~ 1,
    label == 3 ~ 0
...
lr_model_final_wrist_60window <- train(label ~ sd_EDA_wrist + 
                                         sd_ACC_wrist_mag + sd_heart_rate_variability_wrist_BVP + 
                              sd_EDA_wrist*sd_ACC_wrist_mag + sd_EDA_wrist*sd_heart_rate_variability_wrist_BVP + 
                                sd_ACC_wrist_mag*sd_heart_rate_variability_wrist_BVP + 
                                participant #+ participant:sd_heart_rate_variability_wrist_BVP
                             ,
                                  data = train_60window, trControl = train_control, 
                                  method = "glm",
                                  family ="binomial") 
lr_model_final_wrist_chest_60window <- train(label ~ participant + #sd_EDA +# sd_EDA_wrist + 
                                               sd_Temp_chest + 
                                          min_Temp_wrist + min_EMG + sd_inhale + sd_exhale + max_breath + 
                                          i_e_ratio + mean_ACC_chest_mag + sd_ACC_chest_mag + sd_ACC_wrist_mag + 
                                         sd_heart_rate_wrist_BVP + sd_heart_rate_variability_wrist_BVP + 
                                           rms_heart_rate_variability_wrist_BVP + 
                                          sd_heart_rate_chest_ECG + rms_heart_rate_variability_chest_ECG #+ 
                                        # + rms_heart_rate_variability_wrist_BVP*participant + 
                                       +  sd_heart_rate_chest_ECG*participant  
                                         + sd_Temp_chest * min_Temp_wrist
...
summary(lr_model_final_wrist_60window$finalModel)
calc_metrics(lr_model_final_wrist_chest_60window, test_60window, 5)
summary(lr_model_final_wrist_chest_60window$finalModel)
```
...
### Wrist-Only Model
When standard deviation of wrist EDA increases by 1, we are 95% confident that the odds of being stressed versus amused multiply by a factor between `r round(exp(3.6888), 3)` and `r round(exp(3.9632), 3)` with mean `r round(exp(3.826), 3)`, holding all else constant. 
When standard deviation of wrist ACC increases by 1, we are 95% confident that the odds of being stressed versus amused multiply by a factor between `r round(exp(2.76076), 3)` and `r round(exp(4.59924), 3)` with mean `r round(exp(0.445), 3)`, holding all else constant. 
When standard deviation of wrist HRV increases by 1, we are 95% confident that the odds of being stressed versus amused multiply by a factor between `r round(exp(0.3568), 3)` and `r round(exp(0.5332), 3)` with mean `r round(exp(4.027), 3)`, holding all else constant. 

Holding standard deviation of wrist HRV constant, when standard deviation of wrist EDA and wrist ACC both increase by 1, we are 95% confident that the odds of being stressed versus amused multiply by an additional factor between `r round(exp(-1.18976), 3)` and `r round(exp(-1.06824), 3)` with mean `r round(exp(-1.129), 3)`.
Holding standard deviation of wrist ACC constant, when standard deviation of wrist EDA and wrist HRV both increase by 1, we are 95% confident that the odds of being stressed versus amused multiply by an additional factor between `r round(exp(0.33732), 3)` and `r round(exp(0.66268), 3)` with mean `r round(exp(0.5), 3)`.
Holding standard deviation of wrist EDA constant, when standard deviation of wrist HRV and wrist ACC both increase by 1, we are 95% confident that the odds of being stressed versus amused multiply by an additional factor between `r round(exp(0.11188), 3)` and `r round(exp(0.39412), 3)` with mean `r round(exp(0.253), 3)`.

### Chest and Wrist Model

When standard deviation of chest EDA increases by 1, we are 95% confident that the odds of being stressed versus amused multiply by a factor between `r round(exp(1.08432), 3)` and `r round(exp(1.31168), 3)` with mean `r round(exp(1.198), 3)`, holding all else constant. 
When standard deviation of wrist EDA increases by 1, we are 95% confident that the odds of being stressed versus amused multiply by a factor between `r round(exp(8.48892), 3)` and `r round(exp(9.06908), 3)` with mean `r round(exp(8.779), 3)`, holding all else constant. 
When standard deviation of chest temperature increases by 1, we are 95% confident that the odds of being stressed versus amused multiply by a factor between `r round(exp(0.13376), 3)` and `r round(exp(0.20824), 3)` with mean `r round(exp(0.171), 3)`, holding all else constant. 
When minimum wrist temperature increases by 1, we are 95% confident that the odds of being stressed versus amused multiply by a factor between `r round(exp(-1.899), 3)` and `r round(exp(-1.703), 3)` with mean `r round(exp(-1.801), 3)`, holding all else constant. 
When minimum chest EMG increases by 1, we are 95% confident that the odds of being stressed versus amused multiply by a factor between `r round(exp(-0.18416), 3)` and `r round(exp(-0.10184), 3)` with mean `r round(exp(-0.143), 3)`, holding all else constant. 
When standard deviation of inhale increases by 1, we are 95% confident that the odds of being stressed versus amused multiply by a factor between `r round(exp(0.52204), 3)` and `r round(exp(0.62396), 3)` with mean `r round(exp(0.573), 3)`, holding all else constant. 
When standard deviation of exhale increases by 1, we are 95% confident that the odds of being stressed versus amused multiply by a factor between `r round(exp(-0.30716), 3)` and `r round(exp(-0.22484), 3)` with mean `r round(exp(-0.266), 3)`, holding all else constant. 
When maximum breath cycle time increases by 1, we are 95% confident that the odds of being stressed versus amused multiply by a factor between `r round(exp(0.636), 3)` and `r round(exp(0.60268), 3)` with mean `r round(exp(0.66932), 3)`, holding all else constant. 
When inhale/exhale ratio increases by 1, we are 95% confident that the odds of being stressed versus amused multiply by a factor between `r round(exp(-0.32624), 3)` and `r round(exp(-0.25176), 3)` with mean `r round(exp(-0.289), 3)`, holding all else constant. 
When mean chest ACC increases by 1, we are 95% confident that the odds of being stressed versus amused multiply by a factor between `r round(exp(-0.01), 3)` and `r round(exp(0.088), 3)` with mean `r round(exp(0.039), 3)`, holding all else constant. 
When standard deviation of chest ACC increases by 1, we are 95% confident that the odds of being stressed versus amused multiply by a factor between `r round(exp(0.74712), 3)` and `r round(exp(0.85688), 3)` with mean `r round(exp(0.802), 3)`, holding all else constant. 
When standard deviation of wrist ACC increases by 1, we are 95% confident that the odds of being stressed versus amused multiply by a factor between `r round(exp(0.31744), 3)` and `r round(exp(0.45856), 3)` with mean `r round(exp(0.388), 3)`, holding all else constant. 
When standard deviation of wrist HR increases by 1, we are 95% confident that the odds of being stressed versus amused multiply by a factor between `r round(exp(0.16196), 3)` and `r round(exp(0.25604), 3)` with mean `r round(exp(0.209), 3)`, holding all else constant. 
When standard deviation of wrist HRV increases by 1, we are 95% confident that the odds of being stressed versus amused multiply by a factor between `r round(exp(0.1338), 3)` and `r round(exp(0.2122), 3)` with mean `r round(exp(0.173), 3)`, holding all else constant. 
When rms of wrist HRV increases by 1, we are 95% confident that the odds of being stressed versus amused multiply by a factor between `r round(exp(0.17028), 3)` and `r round(exp(0.39371), 3)` with mean `r round(exp(0.282), 3)`, holding all else constant. 
When standard deviation of chest HR increases by 1, we are 95% confident that the odds of being stressed versus amused multiply by a factor between `r round(exp(-0.9262), 3)` and `r round(exp(-0.5538), 3)` with mean `r round(exp(-0.74), 3)`, holding all else constant. 
When rms of chest HR increases by 1, we are 95% confident that the odds of being stressed versus amused multiply by a factor between `r round(exp(0.0218), 3)` and `r round(exp(0.1002), 3)` with mean `r round(exp(0.061), 3)`, holding all else constant. 

Additionally, holding all else constant, when standard deviation of wrist HRV and minimum wrist temperature both increase by 1, we are 95% confident that the odds of being stressed versus amused multiply by an additional factor between `r round(exp(-4.8218), 3)` and `r round(exp(-4.5082), 3)` with mean `r round(exp(-4.665), 3)`.
...
lr_model_final_wrist_coef_tidy_df %>% 
  ...
  kableExtra::kable( booktabs = T,  escape = F, 
                     caption = "Coefficients obtained with Model 1 (Wrist Model)",
...
`lower.ci` = ifelse(term == "S10" | term == "$rms^W_{HRV}$ * S10", "", as.numeric(estimate) - 1.96*as.numeric(std.error)),
  ...
  kableExtra::kable( booktabs = T, escape = F,
                     caption = "Coefficients obtained with  Model 2 (Chest and Wrist Model)",
                     col.names = c("Coefficient", "Estimate", "Std. Error", "p-value", "Lower CI", "Upper CI")) %>%
  #kable_styling(latex_options = c("hold_position")) %>%
