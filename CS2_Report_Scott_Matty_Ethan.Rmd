...
peak_wrist_ACC_Z_plot <- ggplot(all_data, aes(peak_wrist_ACC_Z, color = as.factor(label))) + 
...
mean_ACC_chest_mag_plot <- ggplot(all_data, aes(mean_ACC_chest_mag, color = as.factor(label))) +
  geom_histogram(bindwith=30) +
...
It is also worth discussin independence when evaluating our logistic regression model.  Adding $participant$ addresses independence at the subject level, but each individual sample is not independent; presumably, a person's heart rate from one second ago affects their current heart rate. 
...
In our wrist-only model, the coefficients for standard deviation of wrist electrodermal activity, standard deviation of wrist acceleration, and standard deviation of wrist heart rate variability all suggest that the odds of being stressed increase as these measures increase. However, the negative interaction between standard deviation of wrist electrodermal activity and standard deviation of acceleration suggests that the odds of being stressed don't increase as much when these two measures increase together, holding heart rate variability constant. For a full list of these estimates, please see Table 3. Also, note that these estimates are in terms of the log-odds. 

In the model built with both chest and wrist data, increases in chest standard deviation of electrodermal activity, wrist standard deviation of electrodermal activity, chest standard deviation of temperature, wrist minimum temperature, standard deviation of inhale, maximum breath cycle time, mean chest acceleration, standard deviation of chest acceleration, standard deviation of wrist acceleration, standard deviation of wrist heart rate, standard deviation of wrist heart rate variability, root mean square of wrist heart rate variability, and root mean square of chest heart rate variability all increase the odds that a person is stressed while holding other variables constant. On the other hand, increases in minimum chest electromyography, standard deviation of exhale, inhale/exhale ratio, and standard deviation of chest heart rate all lead to a decrease in the odds that a person is stressed, keeping other variables constant.  For a full list of these estimates, please see Table 4. Also, note that these estimates are in terms of the log-odds. 

In both models, standard deviation of wrist electrodermal activity has the largest estimate. In the wrist-only model, its estimate is 63.622, and in the wrist and chest model, it is 1993.249. This means that in the wrist model, we expect that an increase of 0.01 in the standard deviation of electrodermal activity will cause the odds that a person is stressed to multiply by $e^0.63622$, or 1.889 holding all else constant. In the wrist and chest model, we expect that the same increase of 0.01 in the standard deviation of wrist electrodermal activity will cause the odds that a person is stressed to multiply by $e^19.932$, or 453,492,822 holding all else constant. In this same model, keeping all other variables constant, a 0.01 increase in the standard deviation of chest electrodermal activity will cause the odds that a person is stressed to multiply by $e^0.283$, or 1.328. This suggests that small changes in the standard deviation of electrodermal activity can have very large impacts on the odds that a person is stressed versus amused. As this variability increases, it appears that a given participant is much more likely to be stressed.

We also see a very large negative estimate for the interaction effect between the standard deviation of wrist electrodermal activity and minimum wrist temperature. The estimate of -57.13 implies that for a 0.01 increase in standard deviation of wrist electrodermal activity and an increase of 1 in minimum wrist temperature, we would expect the odds of being stressed versus amused to multiply by $e^{0.604 + 19.932 - 0.5713}$, or 468,567,160, keeping all other variables constant. 
...
In the wrist only model, when compared to the baseline S10 and holding all else constant, we expect the odds of being  stressed for S17 to multiply by exp(0.256), while we expect the odds of being stressed for S7 to multiply by exp(-5.765) and the odds of being stressed for S3 to multiply by exp(-4.34).

For the  sdWACC, when compared to the baseline S10 and holding all else constant,  as SD of ACC magnitude increases by one unit, we expect the odds of being stressed for S7 to multiply by exp(10.803) more and the odds of being stressed for S5 to multiply by exp(6.297) more. 

In the all data model, when compared to the baseline S10 and holding all else constant, we expect the odds of being stressed for S8 to multiply by exp(0.413) and the odds of being stressed for S15 to multiply by exp(0.178), while we  expect the odds of being stressed for the other 12 subjects to multiply by a value less than 1. 
For the sdCHR, when compared to the baseline S10 and holding all else constant, as HR increases by one unit, we expect the odds of being stressed for S11 to multiply by exp(13.179) more and the odds of being  stressed for S17 to multiply by exp(8.979) more; conversely, as HR increases by one unit, we expect the odds of being stressed for S8 to multiply by exp(-4.834).
